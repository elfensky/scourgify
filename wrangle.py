...
def load_config():
    ...
    if os.path.exists(p):
        ...
        for raw in open(p):
            ...
            if ln.startswith("[") and ln.endswith("]"): sec = ln[1:-1].strip(); cfg.setdefault(sec, {}); continue
            if "=" in ln and sec:
...
def setup(cfg):
    ...
    existing = {f"#{lbl}" if not lbl.startswith("#") else lbl: True for lbl in api.field_metadata.custom_field_keys()} \
        if hasattr(api, "field_metadata") else {}
    print("=" * 60); print("calibre-wrangler SETUP — column detection"); print("=" * 60)
    print("FanFicFare custom_cols mapping found:" if fff else "No FanFicFare custom_cols mapping found in this library.")
    for col, fld in sorted(fff.items()): print(f"  {col:16} <- FFF '{fld}'")
    want = {"#fandoms": ("Fandoms", True), "#characters": ("Characters", True), "#genres": ("Genres", True),
            "#relationships": ("Relationships", True), "#status": ("Status", False)}
    have = set(api.all_field_keys()) if hasattr(api, "all_field_keys") else set()
    missing = [c for c in want if c not in have]
    print(f"\nMissing recommended columns: {missing or 'none'}")
    print("(interactive create + config writing runs when launched in a terminal; this is the detect summary.)")

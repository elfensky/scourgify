...
```bash
uv tool install scourgify                             # one dependency: rich
...
Later: `uv tool upgrade scourgify` to update, `uv tool uninstall scourgify` to remove.
[uv](https://docs.astral.sh/uv/) is the only supported installer — it manages the tool's own
environment, so nothing lands in your system Python.
...
- **`overrides/`** *(optional)* — your own files (same formats) that **win over everything** and
  survive upgrades. For personal taste that shouldn't ship.
...
scourgify synopsis --apply --batch 200 # 2b. free + on-device: settle descriptions (see below)
...
## Real descriptions — `scourgify synopsis`
Every book ends up with a **settled** synopsis in Calibre's own description field — the one Calibre
and every reader display.

A description that already tells you what the story is about is **judged and kept, untouched**: no
AI flattening a library of fine author blurbs into one beige voice. Only a useless one ("summary
inside", an update schedule, a tag dump, nothing at all) is replaced, by reading the book's own
prose on-device and writing a spoiler-safe back cover — premise, characters, stakes, a hook, and a
`Themes: …` line. Never the ending.

```bash
scourgify synopsis                          # dry run: what's outstanding. Sends nothing, costs nothing.
scourgify synopsis --apply --batch 200      # settle 200 books (Calibre closed)
scourgify synopsis --apply --step           # ...reviewing each new synopsis before it's written
scourgify synopsis --books 12,40-45 --apply # re-settle named books whatever their stamp says
```

- **Free and on-device** (`--engine apple`, the default) and deliberately slow — around a minute
  for a book it has to write. Run it in chunks; it is designed to chew through a library over weeks.
- Progress lives in the library as the **`#synopsized`** datetime column, so a run always resumes
  where the last one stopped. A story that gains new chapters (`#updated` newer than the stamp) is
  offered again; a book without `#updated` is never re-done automatically.
- Books it cannot read (DRM, a broken file) or the model refuses go to
  `data/synopsis_failures.csv` and leave the queue until you retry them — so the sweep finishes.
- **Turn on FanFicFare's "New Only" for Comments first** (Preferences → Plugins → FanFicFare →
  Customize → Standard Columns), or the next metadata re-fetch overwrites everything this writes.
  The pass refuses to start until you do; `scourgify setup` offers the same fix, and `--force`
  accepts the consequences instead (a clobbered book simply re-enters the queue).
...
- **Scope — which books a run touches** (`select.py` owns this; newest-added-first, so `--batch`/`--limit`
  ...
  they are **synopsis** work: `scourgify synopsis` writes them a real description from the book's own
  prose (free, on-device) and they rejoin the classify backlog on their own. Always dry-run until `--apply`.
- The allowed tag set is the hand-curated `defaults/classify_vocab.txt` **∪ a frequency-gated AO3 seed**
  ...
  seeded term) — editable even for a `uv tool` install, where the bundled files live read-only in site-packages.
